...
export default function AdminUnitsPage() {
  ...
  }, [unitRows, searchQuery, regionFilter, eraFilter, rarityFilter, ownedFilter, imageFilter, sortKey, sortDir]);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted">{adminCardResultLabel(filtered.length, unitRows.length)}</p>
        </div>
        <button
      ...
      <div className="overflow-x-auto rounded-lg border border-border">
        ...
        {filtered.length === 0 && !isLoading && (
          <p className="p-4 text-sm text-muted">No units match your search and filters.</p>
        )}
      </div>
      ) : (
        <>
          ...
          />
          {filtered.length === 0 && !isLoading && (
            <p className="text-sm text-muted">No units match your search and filters.</p>
          )}
        </>
      )}
