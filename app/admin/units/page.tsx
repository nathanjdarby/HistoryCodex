...
export default function AdminUnitsPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (characterId: number) => {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error ?? "Failed to delete unit");
      }
    },
    onSuccess: () => {
  ...
  const unitRows = useMemo(
    () => (characters ?? []).filter((c) => c.cardType === "unit"),
    [characters],
  );

  const filtered = useMemo(() => {
    const rows = unitRows.filter((c) => {
  ...
  }, [unitRows, eraFilter, rarityFilter, ownedFilter, imageFilter, sortKey, sortDir]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-neutral-100">Units</h1>
          <p className="text-sm text-neutral-500">{unitRows.length} total</p>
        </div>
        <button
      ...
      <div className="overflow-x-auto rounded-lg border border-neutral-800">
        ...
        {filtered.length === 0 && !isLoading && (
          <p className="p-4 text-sm text-neutral-500">No units match these filters.</p>
        )}
      </div>
      ) : (
        <>
          ...
          />
          {filtered.length === 0 && !isLoading && (
            <p className="text-sm text-neutral-500">No units match these filters.</p>
          )}
        </>
      )}
